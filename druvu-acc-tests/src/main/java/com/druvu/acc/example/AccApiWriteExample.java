...
/**
 * Example showing how to load a GnuCash file, add an account and a transaction, and save the result to a new file.
 *
...
public class AccApiWriteExample {
    ...
    private void run(Path filePath) throws IOException {
        ...
        log.info(
                "Reloaded {} accounts, {} transactions, {} commodities, {} prices",
                reloaded.accounts().size(),
                reloaded.transactions().size(),
                reloaded.commodities().size(),
                reloaded.prices().size());
