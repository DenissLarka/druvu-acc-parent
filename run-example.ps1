#!/usr/bin/env pwsh
# Run AccApiExample with proper Java module environment
...
mvn -pl druvu-acc-tests dependency:build-classpath "-Dmdep.outputFile=$tempFile" -q
...
$modulePath = @(
    ...
    "druvu-acc-api/target/druvu-acc-api-1.0.0-SNAPSHOT.jar",
    "druvu-acc-gnucash-xml/target/druvu-acc-gnucash-xml-1.0.0-SNAPSHOT.jar",
    $depClasspath.Trim()
) -join [System.IO.Path]::PathSeparator

# Run with module path
Write-Host "Running AccApiExample..." -ForegroundColor Cyan
java --module-path $modulePath --module druvu.acc.examples/com.druvu.acc.example.AccApiExample $GnucashFile
